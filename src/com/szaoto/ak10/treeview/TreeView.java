...
* 修改者，修改日期，修改内容
*/
public class TreeView extends ListView implements OnItemClickListener ,android.widget.AdapterView.OnItemLongClickListener{
	String TAG = "TreeView";
	...
	public TreeView(final Context context, AttributeSet attrs) {
		...
		itemClickCallBack = new LastLevelItemClickListener() {
			...
			public void onLastLevelItemClick(int position,
				...
				+ currentElements.get(position).getTitle()+ " is clicked");
			...
			public void onLastLevelItemLongClick(int position,
				...
				Log.d(TAG, "last level element "
						+ currentElements.get(position).getTitle()+ " is long onclicked");
						Toast.makeText(context,currentElements.get(position).getLevel(), 500).show();
			}
		};
	 ...
	 * @return
	 */
	public synchronized boolean delAllChildElementsByParentId(String parentId) {
	...
	public interface LastLevelItemClickListener {
		...
		//长按事件
		public void onLastLevelItemLongClick(int position, TreeViewAdapter adapter);
	}
	...
	public boolean onItemLongClick(AdapterView<?> parent, View view,
		...
		if (element.isHasChild()) {// 当前节点有子节点时只进行数据显示或隐藏等操作
			...
			itemClickCallBack.onLastLevelItemLongClick(position, adapter);
		}
		return false;
	}
}
